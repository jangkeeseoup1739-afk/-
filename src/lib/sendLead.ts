...
import { IS_LEAD_DELIVERY_ENABLED, LEAD_ENDPOINT } from '../config';
...
/**
 * 관심고객 접수 내용을 구글 앱스 스크립트 웹앱으로 보냅니다.
 *
 * Content-Type 을 text/plain 으로 두는 것은 의도한 것입니다. CORS 사전 요청
 * (preflight) 없이 보내기 위한 것으로, 앱스 스크립트는 본문을 그대로 받아
 * JSON.parse 합니다.
 */
export async function sendLead(lead: CustomerLead): Promise<LeadDeliveryStatus> {
  if (!IS_LEAD_DELIVERY_ENABLED) return 'disabled';
